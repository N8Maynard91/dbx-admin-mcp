/**
 * Function to check the status of a save_url job on Dropbox.
 *
...
const executeFunction = async ({ async_job_id }) => {
  const url = 'https://api.dropboxapi.com/2/files/save_url/check_job_status';
  ...
  try {
    ...
    return { error: 'An error occurred while checking job status.' };
  }
};

/**
 * Tool configuration for checking the status of a save_url job on Dropbox.
 * @type {Object}
 */
const apiTool = {
  ...
  definition: {
    ...
    function: {
      ...
      description: 'Check the status of a save_url job on Dropbox.',
      parameters: {
        type: 'object',
        properties: {
          async_job_id: {
